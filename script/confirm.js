...
module.exports = {
  ...
  onReply: async function ({ message, Reply, event, api, commandName }) {
    ...
    clearTimeout(Reply.unsendTimeout); // Clear the unsend timeout when replied
    ...
    if (targetIDs[0] === "all") {
      ...
      for (let i = 1; i <= lengthList; i++) targetIDs.push(i.toString());
    }
    ...
    if (success.length > 0) {
      api.sendMessage(
        `» The ${args[0] === 'add' ? 'friend request' : 'friend request deletion'} has been processed for ${success.length} people:\n\n${success.join("\n")}` +
        `${failed.length > 0 ? `\n» The following ${failed.length} people encountered errors:\n${failed.join("\n")}` : ""}`,
        event.threadID,
        event.messageID
      );
    } else {
      api.unsendMessage(messageID); // Unsend message if invalid response
      return api.sendMessage("Invalid response. Please provide a valid response.", event.threadID);
    }

    api.unsendMessage(messageID); // Unsend after processing
  },
  ...
  onStart: async function ({ event, api, commandName }) {
    ...
    try {
      ...
      for (const user of listRequest) {
        i++;
        // Fixed time usage: user.time might be undefined, fallback to Date.now()
        ...
        (e, info) => {
          ...
          global.GoatBot.onReply.set(info.messageID, {
            ...
            unsendTimeout: setTimeout(() => {
              api.unsendMessage(info.messageID); // Unsend after countdown
      ...
      console.error(error);
      return api.sendMessage("Failed to fetch friend requests.", event.threadID, event.messageID);
    }
  }
};
